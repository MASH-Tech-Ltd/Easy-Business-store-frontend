...
export default function Design02CheckoutClient({ storeInfo }: { storeInfo?: any }) {
  ...
  if (status === 'success') {
    return (
      <main className="max-w-7xl mx-auto px-8 py-32 flex-1 w-full flex flex-col items-center justify-center text-center">
        ...
        <h1 className="text-4xl font-light tracking-tight text-gray-900 mb-4">{t('orderConfirmed') || 'Order Confirmed'}</h1>
        <p className="text-gray-500 mb-8 max-w-md mx-auto">{t('thankYouPurchase') || 'Thank you for your purchase.'}</p>
        <p className="text-sm font-medium text-gray-400 mb-12">Redirecting to homepage in {countdown} seconds...</p>
        <Link href="/" className="px-8 py-4 bg-gray-900 text-white rounded-md text-sm font-medium hover:bg-gray-800 transition-colors">
  ...
  return (
    <main className="max-w-7xl mx-auto px-8 py-20 flex-1 w-full">
      <h2 className="text-3xl font-light tracking-tight text-gray-900 mb-12">Checkout</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-16">
        ...
        <div className="flex-1 space-y-12">
          ...
          <section>
            <h3 className="text-lg font-medium text-gray-900 mb-6 uppercase tracking-wider">{t('contact') || 'Contact'}</h3>
            <div className="space-y-4">
              <div className="flex flex-col">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t('phoneNumber') || 'Phone Number'}</label>
                <input 
                  type="tel" 
                  value={phone}
                  onChange={(e) => {
                ...
                />
                {fieldErrors.phone?.[0] && <p className="text-xs text-red-500 mt-2">{fieldErrors.phone[0]}</p>}
              </div>
            </div>
          </section>

          <section>
            <h3 className="text-lg font-medium text-gray-900 mb-6 uppercase tracking-wider">{t('personalInfo') || 'Personal Info'}</h3>
            <div className="space-y-6">
              <div className="flex flex-col">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t('fullName') || 'Full Name'}</label>
                <input 
                  type="text" 
                  value={fullName}
                  onChange={(e) => {
                ...
                />
                {fieldErrors.fullName?.[0] && <p className="text-xs text-red-500 mt-2">{fieldErrors.fullName[0]}</p>}
              </div>
              
              <div className="flex flex-col">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t('address') || 'Address'}</label>
                <input 
                  type="text" 
                  value={address}
                  onChange={(e) => {
                ...
                />
                {fieldErrors.address?.[0] && <p className="text-xs text-red-500 mt-2">{fieldErrors.address[0]}</p>}
              </div>

              <div className="flex flex-col">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{t('selectDivision') || 'Division'}</label>
                <select 
                  value={division}
                  onChange={(e) => {
                ...
                >
                  <option value="" disabled>{t('selectDivision') || 'Select Division'}</option>
                  {divisions.map(div => (
                    <option key={div} value={div}>{div}</option>
                  ))}
                </select>
                {fieldErrors.division?.[0] && <p className="text-xs text-red-500 mt-2">{fieldErrors.division[0]}</p>}
              </div>
            </div>
          </section>

          <section>
            <h3 className="text-lg font-medium text-gray-900 mb-6 uppercase tracking-wider">{t('paymentOptions') || 'Payment'}</h3>
            <div className="border border-gray-900 bg-white rounded p-6 flex items-center justify-between shadow-sm cursor-pointer">
              <span className="font-medium text-gray-900">{t('cashOnDelivery') || 'Cash on Delivery'}</span>
        ...
        <div className="lg:w-[420px] shrink-0">
          <div className="bg-gray-50 p-8 rounded-xl sticky top-28">
            <h3 className="text-lg font-medium text-gray-900 mb-8 uppercase tracking-wider">{t('orderSummary') || 'Order Summary'}</h3>
            
            <div className="space-y-6 mb-8 max-h-[400px] overflow-y-auto pr-2">
              {cartItems.length === 0 ? (
                <div className="text-gray-500 text-sm">{t('yourCartIsEmpty') || 'Your Cart is Empty'}</div>
              ) : (
                cartItems.map((item) => (
                  <div key={item.id} className="flex gap-4 items-start">
                    <div className="w-16 h-16 bg-white rounded overflow-hidden shrink-0">
                      {item.image ? (
                        ...
                        <div className="w-full h-full bg-gray-100 flex items-center justify-center"><span className="text-[10px] text-gray-400">No Img</span></div>
                      )}
                    </div>
            ...
            <div className="space-y-4 pt-6 border-t border-gray-200 text-sm">
              ...
              <div className="flex justify-between text-gray-600">
                ...
                <span className="font-medium text-gray-900">৳{totalPrice > 0 ? deliveryCharge.toLocaleString() : 0}</span>
              </div>
            ...
            <div className="mt-8">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">{t('addNote') || 'Note'}</label>
              <textarea 
                value={deliveryNote}
            ...
            >
              {status === 'processing' ? 'Processing...' : (t('confirmOrder') || 'Confirm Order')}
            </button>
