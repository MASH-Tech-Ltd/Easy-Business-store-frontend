...
export default function FilterSidebar05({ categoryId, availableBrands = [] }: { categoryId?: string; availableBrands?: string[] }) {
  ...
  const clearFilters = () => {
    setMinPrice(''); setMaxPrice(''); setBrand(''); setInStock(false);
    const basePath = categoryId ? `/category/${categoryId}` : pathname;
  ...
  return (
    <div className="bg-white sticky top-28">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={clearFilters} className="text-xs font-semibold text-gray-400 hover:text-black transition-colors">Reset</button>
      </div>
      ...
      <form onSubmit={applyFilters} className="space-y-10">
        ...
        {availableBrands.length > 0 && (
          <div>
            <label className="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-4">Brands</label>
            <div className="flex flex-col gap-3">
              {availableBrands.map((b) => (
                <label key={b} className="flex items-center gap-3 cursor-pointer group">
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${brand === b ? 'border-black' : 'border-gray-300 group-hover:border-black'}`}>
                    {brand === b && <div className="w-2 h-2 rounded-full bg-black" />}
                  </div>
                  <input type="radio" name="brand" className="hidden" checked={brand === b} onChange={() => setBrand(b)} onClick={() => { if(brand === b) setBrand('') }} />
                  <span className={`text-sm transition-colors ${brand === b ? 'font-medium text-black' : 'text-gray-500 group-hover:text-black'}`}>{b}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          <label className="flex items-center gap-3 cursor-pointer group">
            ...
            <span className={`text-sm transition-colors ${inStock ? 'font-medium text-black' : 'text-gray-500 group-hover:text-black'}`}>In Stock Only</span>
          </label>
        </div>

        <button type="submit" className="w-full bg-black text-white font-semibold py-3.5 rounded-full text-sm hover:bg-gray-800 transition-colors mt-4">
          Apply Filters
        </button>
      </form>
    </div>
