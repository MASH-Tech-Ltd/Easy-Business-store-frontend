      ...
      <Link href={`/product/${product.slug || product._id || product.id}`} className="group flex bg-[#050505] border border-white/10 hover:border-cyan-400 transition-colors duration-300 w-full rounded-none">
        ...
        <div className="p-6 flex flex-col justify-center flex-1">
          ...
          <div className="mt-auto flex items-end gap-3">
            <span className="font-black text-white text-2xl tracking-tighter">৳{price}</span>
            {hasDiscount && <span className="text-sm text-gray-600 line-through font-mono mb-1">৳{product.originalPrice}</span>}
          </div>
        </div>
      </Link>
    );
      ...
      <Link href={`/product/${product.slug || product._id || product.id}`} className="block">
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-3">
            <span className="font-black text-cyan-400 tracking-tighter text-xl">৳{price}</span>
            {hasDiscount && <span className="text-xs text-gray-600 line-through font-mono">৳{product.originalPrice}</span>}
          </div>
        </div>
      </Link>
